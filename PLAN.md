...
**진행률: 21 / 27 (78%)** (Phase 2에서 항목 하나가 둘로 나뉘어 총 개수가 26→27로 조정됨)
...
## Phase 3 · 연결 고리 구축
- [ ] 영상 설명란/고정댓글 웹사이트 링크 템플릿화 (지금은 `build_description`/`build_pinned_comment`에
  웹사이트 링크가 없음 — 자동 생성 인프라 자체는 이미 있어서 문구만 추가하면 됨)
