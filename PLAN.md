...
## 진행 중 — 영상 퀄리티 개선 (2026-08-21 사용자 피드백)
Phase 2 항목은 전부 체크됐지만, 실제 업로드해본 영상(`video_id: 4DgousvWne0`, 비공개)을 보고
"업로드하기엔 퀄리티가 낮다"는 피드백을 받았습니다. 개선이 필요한 4가지:
- [ ] 배경이 단색이라 심심함 — 그라데이션/이미지·영상 배경 또는 애니메이션 효과 필요
- [ ] 자막이 정적이고 지루함 — 페이드인/슬라이드 애니메이션, 단어 강조, 더 역동적인 타이포그래피
- [ ] 음성/사운드가 빈약함 — 배경음악(BGM), 효과음(딩 소리 등) 추가
- [ ] 전반적인 브랜딩/디자인 톤이 유튜브 숙련 채널만큼 세련되지 않음
→ `video-pipeline/compose_video.py`를 개선 대상으로 작업 예정. 이 4개는 위 27개 체크리스트에는 없는
별도 후속 작업입니다 (Phase 2 자체는 "파이프라인이 작동하는가"를 검증하는 목적이라 완료 처리했고,
"퀄리티가 충분한가"는 별개 트랙으로 여기서 추적합니다).
...
- [x] 사람 검수 단계 프로세스 정의 → `video-pipeline/pipeline.py`가 대본 생성 후 자동 금지어 스캔 + 터미널에 대본 표시 + y/N 확인을 거쳐야 다음 단계(음성/영상 생성)로 진행되도록 구현. 엔드투엔드 3회 실행 검증(경로 버그, 컴플라이언스 오탐 버그 발견 후 수정)
