...
4. **`compose_video.py`** — 9:16(1080x1920) 숏폼 영상 합성:
   ...
   - 사운드: 코드 진행(Am-F-C-G)이 있는 로파이풍 배경음악을 절차적으로 합성해서 낮은 볼륨으로
     깐다. (자막 줄마다 "pop" 효과음도 넣어봤는데, 실제로 들어보니 시청에 방해된다는 피드백을
     받아 뺐다 — `_synthesize_pop`은 코드로는 남아있지만 호출하지 않음. 라이선스 있는 진짜
     음원으로 교체하는 건 후순위 과제.)
   - 자막은 그룹당 최대 30자(대략 2줄)까지 묶어서 보여준다 — 너무 짧으면 박스가 너무 자주
     바뀐다는 피드백을 반영. `_layout_words`가 줄바꿈을 그룹당 한 번만 계산해서, 하이라이트
     단어가 바뀌어도 줄바꿈/박스 크기가 흔들리지 않는다.
